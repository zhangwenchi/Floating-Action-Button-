...
public class FABBackTop extends ImageButton {
    ...
    public FABBackTop(Context context) {
        this(context, null);
    }

    public FABBackTop(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public FABBackTop(Context context, AttributeSet attrs, int defStyle) {
    ...
    private Drawable createDrawable(int color) {
        ...
        if (mShadow && !hasLollipopApi()) {
            Drawable shadowDrawable = getResources().getDrawable(mType == TYPE_NORMAL ? R.drawable.fab_bg_normal
    ...
    private void setBackgroundCompat(Drawable drawable) {
        if (hasLollipopApi()) {
            float elevation;
            if (mShadow) {
                if(getElevation() > 0.0f) elevation =  getElevation();
                else elevation = getDimension(R.dimen.fab_elevation_lollipop);
            } else {
                elevation = 0.0f;
            }
    ...
    public void attachToScrollView(@NonNull mScrollView scrollView) {
    ...
    public void attachToScrollView(@NonNull mScrollView scrollView,
                                   ScrollDirectionListener scrollDirectionListener,
                                   mScrollView.OnScrollChangedListener onScrollChangedListener) {
    ...
    private class ScrollViewScrollDetectorImpl extends ScrollViewScrollDetector {
        ...
        private mScrollView.OnScrollChangedListener mOnScrollChangedListener;

        private void setScrollDirectionListener(ScrollDirectionListener scrollDirectionListener) {
            mScrollDirectionListener = scrollDirectionListener;
        }

        public void setOnScrollChangedListener(mScrollView.OnScrollChangedListener onScrollChangedListener) {
