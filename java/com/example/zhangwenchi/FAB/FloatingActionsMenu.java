...
public class FloatingActionsMenu extends ViewGroup {
  ...
  // the direction of the email icon
  ...
  // the collapse and expand animation
  ...
  // add the email icon to the menu
  ...
  // call it animation( onClick ) outside
  public void toggle() {
    if (mExpanded) {
      collapse();
    } else {
  ...
  // make it expand  or collapse outside
  public void expand() {
    if (!mExpanded) {
      mExpanded = true;
  ...
  // get the state of the email menu
  public boolean isExpanded() {
    return mExpanded;
  }

  // can make the button disable
  @Override
  ...
  private void init(Context context, AttributeSet attributeSet) {
    ...
    mAddButtonColorNormal = attr.getColor(R.styleable.FloatingActionsMenu_fab_addButtonColorNormal, getResources().getColor(android.R.color.holo_blue_dark));
    mAddButtonColorPressed = attr.getColor(R.styleable.FloatingActionsMenu_fab_addButtonColorPressed, getResources().getColor(android.R.color.holo_blue_light));
  ...
  // add the menu button, call once at init time
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    case EXPAND_DOWN:
      ...
      int addButtonY;    // if use "alternative questions" it will be warned at mAddButton.layout()
      if(!expandUp) addButtonY = 0;
      else addButtonY = b - t - mAddButton.getMeasuredHeight();
      ...
      for (int i = mButtonsCount - 1; i >= 0; i--) {
        ...
        int childY;
        if(!expandUp)  childY = nextY;
        else childY = nextY - child.getMeasuredHeight();
        ...
        if (label != null) {
          ...
          if(mLabelsPosition == LABELS_ON_LEFT_SIDE) labelXAwayFromButton = labelsXNearButton - label.getMeasuredWidth();
          else labelXAwayFromButton = labelsXNearButton + label.getMeasuredWidth();

          int labelRight;
          if(mLabelsPosition == LABELS_ON_LEFT_SIDE) labelRight = labelsXNearButton;
          else labelRight = labelXAwayFromButton;

          int labelTop = childY - mLabelsVerticalOffset + (child.getMeasuredHeight() - label.getMeasuredHeight()) / 2;

          int labelLeft;
          if(mLabelsPosition == LABELS_ON_LEFT_SIDE) labelLeft = labelXAwayFromButton;
          else labelLeft = labelsXNearButton;
    ...
    case EXPAND_RIGHT:
      ...
      int addButtonX;
      if(!expandLeft) addButtonX = 0;
      else addButtonX = r - l - mAddButton.getMeasuredWidth();
      // Ensure mAddButton is centered on the line where the buttons should be
      ...
      for (int i = mButtonsCount - 1; i >= 0; i--) {
        ...
        int childX;
        if(!expandLeft) childX = nextX;
        else childX = nextX - child.getMeasuredWidth();
