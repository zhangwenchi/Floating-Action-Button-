...
public class FloatingActionButton extends ImageButton {
  ...
  // google said that we can only use this two size - . 56 & 40
  ...
  // set the icon title can omit it if it is too long
  public void setTitle(String title) {
    if (title.length() > 14) title = "..." + title.substring(10);
    mTitle = title;
  ...
  // this is draw the icon from rectangle to circle
  // draw with bitmap and canvas and use the PorterDuff.Mode.SRC_IN
  // it's important to the resize the bitmap
  public void xiushi(int x){

    final int n = 64; // do not change this value
    int q = n * x;
    int qw = q / 2;

    final Paint paint = new Paint();
    paint.setAntiAlias(true);
    Bitmap target = Bitmap.createBitmap((int)(q), (int)(q), Bitmap.Config.ARGB_8888);
    Canvas canvas = new Canvas(target);
    canvas.drawCircle(qw, qw, qw, paint);
    paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_IN));
    BitmapDrawable bd = (BitmapDrawable)mIconDrawable;
    Bitmap bitmap = bd.getBitmap();
    bitmap = resizeBitmap(bitmap, q, q);
    canvas.drawBitmap(bitmap, 0, 0, paint);

    mIconDrawable =  new BitmapDrawable(getResources(), target);
    updateBackground();
  }
  ...
  /**
   *  the current color for disabled state.
   */
  ...
  Drawable getIconDrawable() {
    if (mIconDrawable != null) {
      ...
      mIconDrawable = getResources().getDrawable(mIcon);
